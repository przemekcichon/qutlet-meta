...
|--------------------|------------------------|---------|-------------------------|-------------|--------------------------------|--------------------|
| Klasa stanu        | `klasa_stanu`          | ACF     | **taxonomy** (single-value, od P-12.2a; wcześniej `select`) | nie | `data.js` `.cls`; `produkt.html:13,46` | **REWIZJA P-12.2a (cutover, D-12.2.1):** pole zapisuje REALNĄ relację `wp_set_object_terms()` z taksonomią §2.2 (`save_terms`/`load_terms` włączone, `return_format=id`, `add_term=0`). Wartość NIE jest już plain literałem w postmeta — konsumenci czytają przez `ClassDefinitionsTaxonomy::for_product(int $product_id): ?array` (§2.2), nie przez `get_field()`/`get_post_meta()`. Historyczny literał (`A`-`D`, `Nowe`) w postmeta zostaje NIETKNIĘTY na dysku (martwy bajt, backfill go nie kasuje) — NIE traktować jako źródło prawdy od tej rewizji. |
