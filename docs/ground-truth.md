...
Zderzenie robimy względem **`docs/kontrakt-danych.md`** (prawda *zamierzona*: nazwy
pól, meta keys, kształty). Gdy realny kod rozjeżdża się z kontraktem — NIE godzimy
tego po cichu: **STOP**, opis rozbieżności, decyzja użytkownika (poprawka w kodzie
czy w kontrakcie). Kod = prawda ostateczna o stanie; kontrakt = prawda zamierzona;
kierunek naprawy wybiera człowiek. (Patrz CLAUDE.md → „Realizacja punktu planu".)
...
przykład ustalimy w toku pracy nad pierwszymi punktami planu, żeby nie wymyślać go z pamięci.
